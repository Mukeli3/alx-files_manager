...
class RedisClient {
  constructor() {
    this.client = redis.createClient({
      // set host config for a db connection, with a fallback value if the env var DB_HOST is not set.
      host: process.env.DB_HOST || 'localhost',
      port: process.env.DB_PORT || 6379, // set Redis port, default port
    });
    // transform callback funcs of any redis func to an async method (promise-based function) -> promisify
...
/**
 * Takes a string key, value and duration in sec as args to store in Redis (expiration set by the duration arg)
